...
erDiagram
    ...
    OSSERVAZIONE {
        ...
        datetime timestamp
        float durata_minuti
        string condizioni_meteo
        string note
    }

    TELESCOPIO ||--o{ OSSERVAZIONE : effettua
    CORPO_CELESTE ||--o{ OSSERVAZIONE : oggetto_di
    RICERCATORE }o--o{ OSSERVAZIONE : conduce
```
...
2. Crea una applicazione Flask con le seguenti pagine:
   ...
   - `/telescopi`: mostra una tabella con la lista dei telescopi ordinati per apertura
   - `/corpi`: mostra una tabella con la lista dei corpi celesti ordinati per distanza
   - `/ricercatore/<int:id>`: lista delle osservazioni condotte dal ricercatore con relativi corpi celesti (guarda [link](https://flask.palletsprojects.com/en/stable/quickstart/#unique-urls-redirection-behavior))
3. Crea una pagina `/statistiche` con il risultato delle seguenti query:
